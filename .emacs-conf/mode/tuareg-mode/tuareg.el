...
(defconst tuareg-mode-version "Tuareg Version 1.41.2"
  "        Copyright © 1997-2004 Albert Cohen, all rights reserved.
...
(if (and (featurep 'font-lock)
	...
	'(("\\<\\(external\\|open\\|include\\)\\>"
	  ...
	  ("\\<\\(val\\|type\\|method\\|constraint\\|class\\|in\\|inherit\\|initializer\\|let\\|rec\\|and\\|begin\\|object\\|end\\)\\>"
	   0 'tuareg-font-lock-governing-face nil)
	  ("\\<\\(as\\|do\\(ne\\|wnto\\)?\\|else\\|for\\|if\\|let\\|m\\(atch\\|utable\\)\\|new\\|p\\(arser\\|rivate\\)\\|t\\(hen\\|o\\|ry\\)\\|w\\(h\\(en\\|ile\\)\\|ith\\)\\|lazy\\|virtual\\|exception\\|raise\\|failwith\\|exit\\|assert\\|fun\\(ction\\)?\\)\\>"
	   0 'font-lock-keyword-face nil)
	  ("\\<\\(\\(method\\([ \t\n]+private\\)?\\)\\([ \t\n]+virtual\\)?\\|val\\([ \t\n]+mutable\\)?\\|constraint\\|external\\|and\\|let\\([ \t\n]+rec\\)?\\)\\>[ \t]*\\(['_a-z\277-\377]\\(\\w\\|[._]\\)*\\)"
	   7 'font-lock-function-name-face nil)
	  ("\\<\\(\\(method\\([ \t\n]+private\\)?\\)\\([ \t\n]+virtual\\)?\\|val\\([ \t\n]+mutable\\)?\\|constraint\\|external\\|and\\|let\\([ \t\n]+rec\\)?\\)\\>[ \t]*\\(['_a-z\277-\377]\\(\\w\\|[._]\\)*\\)\\>[ \t]*\\(\\(\\w\\|[->_ \t,?~:.]\\|(\\(\\w\\|[->_ \t,?~:.=]\\)*)\\)*\\)"
	   9 'font-lock-variable-name-face nil)
	  ("\\<\\(\\(method\\([ \t\n]+private\\)?\\)\\([ \t\n]+virtual\\)?\\|val\\([ \t\n]+mutable\\)?\\|constraint\\|external\\|and\\|let\\([ \t\n]+rec\\)?\\)\\>[ \t]*\\(\\(\\w\\|[->_ \t,?~:.]\\|(\\(\\w\\|[->_ \t,?~:.=]\\)*)\\)*\\)"
	   7 'font-lock-variable-name-face nil)
	  ("\\<\\(open\\|\\(class\\([ \t\n]+type\\)?\\)\\([ \t\n]+virtual\\)?\\|inherit\\|module\\([ \t\n]+type\\)?\\|type\\)\\>[ \t]*\\(\\(\\<['_A-Za-z\277-\377~?]\\w*\\>[ \t]*\\)*\\<['_A-Za-z\277-\377~?]\\w*\\>:?\\)\\>"
	  ...
	  ("[][;,()|{}@^!:*=<>&/%+~?---]\\.?\\|\\.\\.\\.*"
	   0 'tuareg-font-lock-operator-face t))
...
(when tuareg-with-caml-mode-p
  ...
  (define-key tuareg-mode-map [mouse-2] 'caml-types-mouse-ignore)
  (define-key tuareg-mode-map [down-mouse-2] 'caml-types-explore)
  ;; caml-help
...
Known bugs:
  - With Emacs: text highlighting may be incorrect in some tricky examples
    ...
    with the improved XEmacs Font-Lock interface.
    ...
    (cond ((or (string= kwop "type") (string= kwop "module"))
	   (let ((kwop2 (tuareg-find-meaningful-word)))
	     (cond ((string= kwop2 "with") kwop2)
		   ...
		   ((or (string= kwop "module") (string= kwop2 "let")) kwop2)
...
(defconst tuareg-find-->-match-regexp
  (tuareg-make-find-kwop-regexp "\\<\\(external\\|val\\|method\\|let\\|with\\|fun\\(ction\\)?\\|parser\\)\\>\\|[|:;]"))
(defun tuareg-find-->-match ()
		...
		(let ((current-column-module-type nil))
		  (+
		   ...
		   (if current-column-module-type
		     ...
		     (current-column))))
	      (if (save-excursion (tuareg-find-=-match)
				  (looking-at "\\<sig\\>"))
		  (prog2
		      (tuareg-back-to-paren-or-indentation t)
...
(defconst tuareg-beginning-phrase-regexp
  "^#[ \t]*[a-z][_a-z]*\\>\\|\\<\\(end\\|val\\|type\\|module\\|sig\\|struct\\|class\\|exception\\|open\\|let\\)\\>\\|;;"
  "Regexp matching tuareg phrase delimitors.")
